        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <el-button type="danger"  @click="onClickForDeleteArticle(scope.row._id)" plain size="mini" >删除</el-button>
                    <!-- :disabled="!$store.state.authList.includes('删除文章')" -->
                </template>
            </el-table-column>
        </el-table>
        ...
        methods:{
            ...
            onClickForDeleteArticle(_id){
                this.$confirm('永久删除文章, 是否继续?', '提示', {
                    ...
                    let {status,data:{code,msg}} = await this.$http.post(`/api/article/delete`,{_id})
                    if(status===200){
                        if(code===0){
                            this.$message({
                                message: '删除文章成功',
            ...
            async getAllArticle(){
                let {status,data:{code,msg,articleList,count}} = await this.$http.get(`/api/article/list?keyWord=${this.keyWord}&pageNum=${this.pageNum}&pageSize=${this.pageSize}`)
                if (status===200){
                    if (code===0){
