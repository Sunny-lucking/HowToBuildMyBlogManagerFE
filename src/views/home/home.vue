  ...
  methods: {
    getCheckArr(list) {
      ...
      list.forEach((tempItem1) => {
        if (tempItem1.is_enable === true) tempArr.push(tempItem1.authName);
        tempItem1.children.forEach((tempItem2) => {
          if (tempItem2.is_enable === true) tempArr.push(tempItem2.authName);
          tempItem2.children.forEach((tempItem3) => {
            if (tempItem3.is_enable === true) tempArr.push(tempItem3.authName);
          });
        });
      });
    ...
    async getSelfRole() {
      let {
        status,
        data: { role },
      } = await this.$http.get("/api/role/getSelfRole");
    ...
    AdminLogin() {
      if (localStorage.blogBackToken) {
        const token = localStorage.blogBackToken;
        // 解析token
        const decode = jwt_decode(token);
        console.log(decode);
        this.$store.commit("setAdmin",decode)
      }
    },
  },
  mounted() {
